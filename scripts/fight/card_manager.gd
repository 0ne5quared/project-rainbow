...
func _on_recieved_packet(packet: Dictionary) -> void:
	if (
		packet.type != ConnectionManager.GameMessage.NEW_CARD
		or packet.card.id in _cards
	):
		return

	add_card(
		packet.card.data as Dictionary,
...
func move_card(card_id: String, zone: Card.Zone) -> void:
	...
	if zone in Card.PUBLIC_ZONE:
		ConnectionManager.send(
			ConnectionManager.GameMessage.NEW_CARD, {card = card.as_dict()}
		)
